  ...
  return (
    <>
      <nav
        className={`${
          open ? 'w-64' : 'w-24'
        } z-10 flex flex-col items-center justify-between py-4 px-2 h-screen bg-blue-500 gap-4 transition-all ease-in-out duration-150`}
      ...
      >
        ...
        <div
          className={`${
            open ? 'flex' : 'hidden'
          } flex-col items-center justify-start w-full h-fit gap-1`}
        >
          {opcoes.map((opcao) => (
            <OpcaoBarra
              key={opcao.texto}
